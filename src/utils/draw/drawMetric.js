...
export function drawMetric(ctx, settings) {
  ...
  if (patternType === 'reference') {
    drawMetricReference(ctx, w, h, gridInterval, gridSubdivision, lineStroke, textSize, title, settings)
    return
  }
  ...
  // 4. Border labels (meter markers)
  drawBorderLabels(ctx, w, h, gridInterval, gridSubdivision, wall, textSize, colors.text)

  // 5. Registration marks (corners + center)
...
function drawBorderLabels(ctx, w, h, gridInterval, gridSubdivision, wall, textSize, textColor) {
  if (!gridInterval || gridInterval <= 0) return
  ctx.fillStyle = textColor
  ctx.font = `${textSize}px monospace`

  // Top edge — X axis (horizontal positions)
  ctx.textAlign = 'center'
  ctx.textBaseline = 'top'
  for (let i = 0; ; i++) {
    const x = i * gridInterval
    const val = +(i * gridSubdivision).toFixed(6)
    if (val > wall.width + 0.001) break
    ctx.fillText(formatMetric(val), x, 4)
  }

  // Left edge — Y axis (vertical positions)
  ctx.textAlign = 'left'
  ctx.textBaseline = 'middle'
  for (let j = 1; ; j++) {   // start at 1 — skip 0 to avoid corner overlap with X label
    const y = j * gridInterval
    const val = +(j * gridSubdivision).toFixed(6)
    if (val > wall.height + 0.001) break
    ctx.fillText(formatMetric(val), 4, y)
  }
}

function drawMetricReference(ctx, w, h, gridInterval, gridSubdivision, lineStroke, textSize, title, settings) {
